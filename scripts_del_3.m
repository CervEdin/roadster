...
test_graph.x = linspace(-1, total_consumption(max(anna.distance_km), anna.string, 2^16) + 1);
test_graph.y = arrayfun(@(x) reach(x, 'speed_anna'), test_graph.x);
plot(test_graph.x, test_graph.y)
hold on
test_graph.x = linspace(-1, total_consumption(max(elsa.distance_km), elsa.string, 2^16) + 1);
