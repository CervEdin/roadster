...
%test_graph.y = arrayfun(@(x) time_to_destination_simpson(x, 'speed_anna', 2^16), test_graph.x);
...
% test_graph.y = arrayfun(@(x) time_to_destination_simpson(x, 'speed_elsa', 2^16), test_graph.x);
% plot(test_graph.x, test_graph.y)
% xlabel('distance (km)')
% ylabel('time (hours)')
